...
class ModbusUSBDriver(SmapDriver):
    def setup(self, opts):
        ##Get information for config file about meters to be polled and the rates at which they are to polled. 
        ##Also, get meter specific Metadata that is supplied by the config file.
	...
	self.parameters = {  'EM6400' : ['Power', 'PowerFactor', 'Voltage','Frequency','PowerPhase1','PowerFactorPhase1','PowerPhase2','PowerFactorPhase2','PowerPhase3','PowerFactorPhase3','Energy'],
			'EM6433' : ['Power', 'PowerPhase1','PowerPhase2','PowerPhase3','Energy'],
			'EM6436' : ['Power','PowerPhase1','PowerFactorPhase1','PowerPhase2','PowerFactorPhase2','PowerPhase3','PowerFactorPhase3','Energy' ] }
	...
	self.units = {'Power' : 'Watts',
        ...
        ##Generate lists with strings corresponding to Meters Ids.
        #self.CURRENT = ['/Meter' + str(x.Id) + '/Current' for x in meters]
        #self.POWER = ['/Meter' + str(x.Id) + '/Power' for x in meters]
        #self.ENERGY = ['/Meter' + str(x.Id) + '/Energy' for x in meters]
        ...
        ##Create 60 queues each representing a second. If the nth queue contains a meter ID x ==> That meter x is to be polled in the nth second.
    ...
    def read(self):
		...
		self.add('/Meter' + str(x.Id) + '/' + self.parameters[x.Model][y], t, value)
            ...
            #power = convert((self.res.registers[1]<<16) + self.res.registers[0])
            #amp = convert((self.res.registers[11]<<16) + self.res.registers[10])
            #energy = convert((self.res.registers[59]<<16) + self.res.registers[58])

            #self.add('/Meter' + str(x.Id) + '/Current', t, amp)
            #self.add('/Meter' + str(x.Id) + '/Power', t, power)
            #self.add('/Meter' + str(x.Id) + '/Energy', t, energy)
    ...
    def CONNECT_TO_METER(self):
        ...
        try:
            clt = None
            METER_PORT = self.find_tty_usb(self.ID_VENDOR, self.ID_PRODUCT)        #reading to which port rs485(client) is connected
            clt = ModbusClient(retries = self.RETRIES, method=self.COM_METHOD, port=METER_PORT, baudrate=self.BAUD_RATE, stopbits=self.STOP_BITS, parity=self.PARITY, bytesize=self.BYTE_SIZE,timeout=self.TIME_OUT) 
            self.iftrue = clt.connect()
            print self.iftrue, clt
            return clt
